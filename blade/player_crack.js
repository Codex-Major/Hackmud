function(context,args) { // help:false, user:"", alt:"", target:#s.beta.lock_sim
	...
	if(args.help){
		var ht=`
		T 1-2  Exempli Gratia: 
						  blade.player_crack{user:"", alt:"", target:#s.example.loc}
	  
		  [*] WIP BREAKS ON CON_SPEC AND ACCT_NT`
	#D(ht)
	return		
	}
...
if(l.includes('sn_w_glock')){
	...
	if(args.alt || balance==0){
		...
		if(bal==0){
			...
			for(var i in snw){
				if(l in snw){
					#ms.codex.bank({amount:snw[l],user:args.user,pass:"sdkfbSUE325"})
					g.sn_w_glock=snw[l]
					c()
					break
				}
